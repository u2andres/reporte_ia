...
class ReportMin02
{
  ...
  { // Pdf creation...
    ...
    // ...
    $dir1yaml  = dirname(__FILE__).'/config/';
  ...
  static public function mk_header($pdf)
  {
    ...
    // // ...
    // $c_val2ck = "\n" . '$h_header : ' . print_r( $h_header, TRUE);
    // $c_file = dirname(__FILE__).'/../../../../../log/info-debug.txt';
  ...
  { // // completar el footer...
    ...
    // ...
    $margin1left = 10;    
    $pg_size     = $h_user['pg_size'];
    
    // ...
    $y = $pdf->GetY();

    // NO LO IMPRIME, VER QUE PASA!!!
    // ------------------------------
    // linea... 
    $pdf->Line($margin1left, $y - 50, $margin1left + $pg_size, $y - 50);
    
    // fecha y hora, a izquierda...
    $fecha = 'Fecha ' . self::adj_fecha(date('c')) . ' - Hora ' . date('H:i');
    $pdf->writeHTMLCell($pg_size / 2, 0, $margin1left, $y, $fecha, 0, 0, 0, true, 'L');
    ...
    $pdf->writeHTMLCell($pg_size / 2, 0, $margin1left, $y, $title . ' - ' . $pg, 0, 0, 0, true, 'R');
  }
